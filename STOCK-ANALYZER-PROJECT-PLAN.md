...
- Week1 / Week2：已完成并可运行（市场看板、K线、实时行情联动）。
- Week3：已完成（AI信号、风控、选股联动、通知模块）。
- Week4：主流程已落地（持仓管理、回测、仿真盘、设置页、构建脚本）。
- 选股模块近期更新：
  - 修复策略下拉切换后不自动刷新问题（切换即重跑）。
  - 结果条数改为可选：Top 10 / 20 / 50 / 120（默认 Top 10）。
  - 后端 limit 限制为 1~120，避免无上限大结果。
  - 收紧三套预设条件，减少噪声候选。
  - 增加盘后全市场日线补齐，避免数据池不完整导致“选不到”。

- 当前进度结论：
  - 代码开发状态：进行中（核心功能可用，持续优化中）。
